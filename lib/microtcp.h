 ...
 */
typedef struct /*this represents a socket that runs tcp  */
{
  ...
  size_t cwnd;     /*conjestion window is dynamically changed during runtime  through the transmitting of packets */
  size_t ssthresh; /*it shows when the slow start phase stops  and when conjestion avoidance starts*/
...
typedef struct
{
  ...
  char payload[MICROTCP_MSS]; // Assuming payload size is up to MICROTCP_MSS
} buffered_packet_t;

#define MAX_BUFFERED_PACKETS 10 // Adjust this number based on expected window size
buffered_packet_t out_of_order_buffer[MAX_BUFFERED_PACKETS];
int num_buffered_packets = 0;
ssize_t minimum(size_t a, size_t b, size_t c);
ssize_t minimum2(ssize_t a, ssize_t b);
enum condition
{
  TIMEOUT,
  DUPLICATE3,
...
int init_shutdown(microtcp_sock_t *socket);
int check_send_conditions(microtcp_sock_t *socket, microtcp_header_t *me, size_t buffer_size) ;
microtcp_sock_t
